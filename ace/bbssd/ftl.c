...
QemuLogFile *error_log;
QemuLogFile *voltage_log;
gchar const *dir1 = "./logging";
gchar const *dir2 = "./logging";
gchar *error_f_path = NULL;
gchar *vol_f_path = NULL;

uint64_t total_ecc_uber=0;
uint64_t total_read_uber=0;
uint64_t total_rber=0;
...
static void ssd_init_nand_page(struct nand_page *pg, struct ssdparams *spp)
{
    ...
    pg->sec = g_malloc0(sizeof(struct nand_sec) * pg->nsecs);
    pg->time=0;
    for (int i = 0; i < pg->nsecs; i++) {
        ssd_init_nand_sec(&pg->sec[i],spp);
    }
    init_TLC_state(&pg->TLC_states);
    init_MLC_state(&pg->MLC_states);
    pg->status = PG_FREE;
    pg->bch = init_bch(spp->bch_n,spp->bch_t,0);
...
static void ssd_init_nand_plane(struct nand_plane *pl, struct ssdparams *spp)
{
    ...
    for (int i = 0; i < pl->nblks; i++) {
        ...
        init_percent ++;
        ftl_log("initialize SSD [%d/%d]\n",init_percent,spp->tt_pls * spp->blks_per_pl);
    }
}
...
static int t = 0;
static void* allocate_ssd_nand(void *arg){
    struct nand_thread *n = (struct nand_thread *)arg;
    struct ssdparams *spp = &n->sp;
    int i = n->ch_idx;
    struct ssd_channel *ch = n->ch;

    femu_log("allocate %d ch start\n",i);
    ch->nluns = spp->luns_per_ch;
    ch->lun = g_malloc0(sizeof(struct nand_lun) * ch->nluns);
    for (int i = 0; i < ch->nluns; i++) {
        ssd_init_nand_lun(&ch->lun[i], spp);
    }
    femu_log("allocate ch %d done\n",i);
     ch->next_ch_avail_time = 0;
     ch->busy = 0;
     t++;
...
void ssd_init(FemuCtrl *n)
{
    ...
    ssd_init_params(spp);
    struct nand_thread nand_init[spp->nchs];

    /* initialize ssd internal layout architecture */
    ssd->ch = g_malloc0(sizeof(struct ssd_channel) * spp->nchs);
    for (int i = 0; i < spp->nchs; i++) {
        nand_init[i].sp = *spp;
        nand_init[i].ch_idx=i;
        nand_init[i].ch=&ssd->ch[i];
        qemu_thread_create(&nand_init[i].nand_init, "FEMU-NAND-Thread", allocate_ssd_nand, &nand_init[i],
    ...
    error_f_path = g_build_filename(dir1, "ace_error.log", NULL);
    vol_f_path = g_build_filename(dir2, "ace_voltage.log", NULL);

    qemu_thread_create(&ssd->ftl_thread, "FEMU-FTL-Thread", ftl_thread, n,
                       QEMU_THREAD_JOINABLE);
    femu_log("End ssd initialize... Start ACE...\n");
}
...
static uint64_t gc_write_page(struct ssd *ssd, struct ppa *old_ppa)
{
    ...
    old_pg = get_pg(ssd,old_ppa);
    new_pg = get_pg(ssd, &new_ppa);

    for(int i=0; i <spp->secs_per_pg; i++){
        if(old_pg->sec[i].check == 1){
            for(int j=0; j < spp->oob_ecc_len; j++){
                    (new_pg->sec[i]).ecc[j]=(old_pg->sec[i]).ecc[j];
                    (old_pg->sec[i]).ecc[j] = 0;
            } 
        } 
        (old_pg->sec[i]).check=0;
        (new_pg->sec[i]).check=0;
        (new_pg->sec[i]).before_buf=(old_pg->sec[i]).before_buf;
    }
    

    new_pg->time = qemu_clock_get_ms(QEMU_CLOCK_REALTIME);
    old_pg->time = 0;
...
static uint64_t ssd_write(struct ssd *ssd, NvmeRequest *req, FemuCtrl *n)
{
    ...
    for (lpn = start_lpn; lpn <= end_lpn; lpn++) {
        ...
        pg = get_pg(ssd,&ppa);
        pg->time = qemu_clock_get_ms(QEMU_CLOCK_REALTIME);
        bch = pg->bch;
        for(int i=0; i < spp->secs_per_pg; i++){
            sec = &pg->sec[i];
            sec->before_buf = buf;
