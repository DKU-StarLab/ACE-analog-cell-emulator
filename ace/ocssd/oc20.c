                                ...
                                int *nr)
{
    ...
    for (int i = 0; i < nlb; i++) {
        ...
        if (cur_pg_addr == prev_pg_addr) {
            ...
            //femu_log("page type: %d\n",bucket[secs_idx].page_type);
...
static int oc20_advance_status(FemuCtrl *n, NvmeNamespace *ns, NvmeCmd *cmd,
                               NvmeRequest *req, uint64_t *lbal, int req_index)
{
    Oc20Namespace *lns = ns->state;
    Oc20RwCmd *ocrw = (Oc20RwCmd *)cmd;
    ...
    //void *buf = (n->mbe)->logical_space;
    //uint64_t mb_opt=lbal[0];
    ...
    for (int i = 0; i < secs_idx; i++) {
        ...
        pg = addr_bucket[i].pg;
        //femu_log("ch: %d lun: %d page: %d secs_idx: %d pg:%d page_type: %d\n",ch,lun,pg,addr_bucket[i].pg,secs_idx,page_type);
        lunid = ch * num_lun + lun;
        idx_wear_out = (((ch+1)*(lun+1)*(pg+1)*(i+1)))%(100000);

        while((memory_index < req_index) & !(req->is_write) & (err_idx < (req_index / secs_idx))){
            //femu_log("secs_idx: %d memory index:%d\n",secs_idx,req_index);

            #if 0
            err_cnt += TLC_nand_sec_error(buf,lns->wear_out,idx_wear_out);
            #endif

            #if 0
                MLC_nand_sec_error(buf,lns->wear_out,idx_wear_out);
            #endif
            total += (48/cell_mode);
            memory_index ++;
            err_idx ++;
            idx_wear_out += (48/cell_mode);
            if(idx_wear_out >= 100000)
                idx_wear_out = 0;
            //mb_opt = lbal[memory_index];
            //buf = (n->mbe)->logical_space + mb_opt;
            //femu_log("%d %d %lf\n",err_cnt,total,(float)err_cnt/total);
        //fprintf(fp,"%d %d %lf\n",err_cnt,total,(float)err_cnt/total);
        }
        err_idx=0;
...
static uint16_t oc20_rw(FemuCtrl *n, NvmeCmd *cmd, NvmeRequest *req, bool vector)
{
    ...
    oc20_advance_status(n, ns, cmd, req, aio_sector_list,index);
...
static int oc20_init_namespace(FemuCtrl *n, NvmeNamespace *ns, Error **errp)
{
    ...
    // int read=1000000;
    // int time=8640;
    // fp1=fopen("cycle3000.txt","a");
    // fp2=fopen("read1000000.txt","a");
    // fp3=fopen("timeyear.txt","a");
